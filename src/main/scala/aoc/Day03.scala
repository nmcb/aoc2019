...
object Day03 {
  ...
  def intersections(w1: String, w2: String): Seq[Pos] =
    wire(w1).toSet.intersect(wire(w2).toSet).toSeq

  def distances(w1: String, w2: String): Seq[Pos] =
    intersections(w1, w2).sortWith((p1, p2) => distance(p1) < distance(p2))

  assert(distance(distances("R75,D30,R83,U83,L12,D49,R71,U7,L72","U62,R66,U55,R34,D71,R55,D58,R83")
    .head) == 159)
  assert(distance(distances("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51","U98,R91,D20,R16,D67,R40,U7,R15,U6,R7")
    .head) == 135)
  ...
  def intersectOnPosOrderByCombinedStep(w1: Seq[(Pos, Int)], w2: Seq[(Pos, Int)]): (Pos, Int) = {
  ...
  assert { 
    ...
    intersectOnPosOrderByCombinedStep(w1, w2)._2 == 610
  }
  ...
  assert { 
    ...
    intersectOnPosOrderByCombinedStep(w1, w2)._2 == 410
  }

  val result2: (Pos, Int) =
    intersectOnPosOrderByCombinedStep(wire(desc1).zipWithIndex, wire(desc2).zipWithIndex)
